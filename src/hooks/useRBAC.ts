...
const DEFAULT_ROLE_PERMISSIONS: RolePermissions[] = [
  ...
  },
  {
    role: Role.ADMIN,
    permissions: [
      Permission.EMAIL_READ, Permission.EMAIL_WRITE, Permission.EMAIL_SEND, Permission.EMAIL_DELETE,
      Permission.CONTACTS_READ, Permission.CONTACTS_WRITE, Permission.CONTACTS_IMPORT, Permission.CONTACTS_EXPORT,
      Permission.CALENDAR_READ, Permission.CALENDAR_WRITE, Permission.CALENDAR_SHARE,
      Permission.SETTINGS_READ, Permission.SETTINGS_WRITE,
      Permission.TEAM_READ, Permission.TEAM_MANAGE, Permission.TEAM_INVITE, Permission.TEAM_ASSIGN_ROLES,
      Permission.ADMIN_PANEL_ACCESS, Permission.ADMIN_USER_MANAGE, Permission.ADMIN_AUDIT_LOGS,
      Permission.REPORTS_VIEW, Permission.REPORTS_EXPORT, Permission.REPORTS_ANALYTICS,
      Permission.INTEGRATIONS_READ, Permission.INTEGRATIONS_CONNECT, Permission.INTEGRATIONS_MANAGE
    ],
  ...
  },
  {
    role: Role.MANAGER,
    permissions: [
      Permission.EMAIL_READ, Permission.EMAIL_WRITE, Permission.EMAIL_SEND, Permission.EMAIL_ARCHIVE,
  ...
  },
  {
    role: Role.MEMBER,
    permissions: [
      Permission.EMAIL_READ, Permission.EMAIL_WRITE, Permission.EMAIL_SEND, Permission.EMAIL_REPLY,
      Permission.CONTACTS_READ, Permission.CONTACTS_WRITE,
      Permission.CALENDAR_READ, Permission.CALENDAR_WRITE,
      Permission.SETTINGS_READ
    ],
    displayName: 'Member',
...
const ROLE_HIERARCHY: RoleHierarchy[] = [
  { role: Role.SUPER_ADMIN, level: 5, canAssignTo: [Role.SUPER_ADMIN, Role.ADMIN, Role.MANAGER, Role.MEMBER, Role.VIEWER, Role.GUEST] },
...
const generateMockCustomPermissionSets = (): CustomPermissionSet[] => [
  ...
  },
  {
    ...
    permissions: [
      Permission.EMAIL_READ,
      Permission.CONTACTS_READ, Permission.CONTACTS_EXPORT,
      Permission.CALENDAR_READ,
      Permission.SETTINGS_READ,
      Permission.REPORTS_VIEW, Permission.REPORTS_EXPORT, Permission.REPORTS_ANALYTICS
    ],
...
export const useRBAC = () => {
  ...
  const hasAllPermissions = useCallback((userId: string, permissions: Permission[]): boolean => {
    return permissions.every(perm => hasPermission(userId, perm).hasPermission);
  }, [hasPermission]);

  const hasAnyPermission = useCallback((userId: string, permissions: Permission[]): boolean => {
  ...
  const isHigherRole = useCallback((role1: Role, role2: Role): boolean => {
  ...
  const createCustomPermissionSet = useCallback(async (payload: CreateRolePayload): Promise<CustomPermissionSet | null> => {
  ...
  const createAccessPolicy = useCallback(async (policy: Omit<AccessPolicy, 'id'>): Promise<AccessPolicy | null> => {
  ...
  // Settings management
  const updateSettings = useCallback(async (updates: Partial<RBACSettings>): Promise<RBACSettings> => {
  ...
  const stats = useMemo((): RBACStats => {
    const usersByRole = userRoleAssignments.reduce((acc, assignment) => {
  ...
  // Utility functions
  const getUsersByRole = useCallback((role: Role): UserRoleAssignment[] => {
  ...
  const getFilteredAuditLogs = useCallback((filter?: { userId?: string; action?: string; startDate?: Date; endDate?: Date }): PermissionAuditLog[] => {
