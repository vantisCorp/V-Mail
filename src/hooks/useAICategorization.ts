...
export const useAICategorization = (
  config: Partial<CategorizationModelConfig> = {}
) => {
   ...
   */
  const categorizeEmail = useCallback(async (email: any): Promise<CategorizationResult> => {
    if (!isEnabled) {
  ...
  const addTrainingExample = useCallback((
    example: Omit<TrainingExample, 'timestamp'>
  ): void => {
  ...
  const getStatistics = useCallback((): CategorizationStats => {
    ...
    const processingTimeAvg = totalEmailsCategorized > 0
      ? totalProcessingTime / totalEmailsCategorized
      : 0;
    ...
    return {
      ...
      lastTrainingDate: trainingExamples.length > 0
   ...
   */
  const recategorizeEmail = useCallback(async (
    emailId: string,
  ...
  const updateConfig = useCallback((
    updates: Partial<CategorizationModelConfig>
  ): void => {
