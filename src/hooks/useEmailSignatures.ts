...
export function useEmailSignatures() {
  ...
  const [templates] = useState<SignatureTemplate[]>(DEFAULT_TEMPLATES);
  ...
  const saveUsageLogs = useCallback(
    (logs: SignatureUsageLog[] | ((prev: SignatureUsageLog[]) => SignatureUsageLog[])) => {
  ...
  const createSignature = useCallback(
    async (payload: CreateSignaturePayload): Promise<EmailSignature> => {
  ...
  const updateSignature = useCallback(
    async (id: string, payload: UpdateSignaturePayload): Promise<EmailSignature | null> => {
  ...
  const toggleSignatureActive = useCallback(
    async (id: string, active?: boolean): Promise<boolean> => {
      ...
      saveSignatures((prev) => {
        ...
        return prev.map((s) =>
          s.id === id
            ? { ...s, isActive: active !== undefined ? active : !s.isActive, updatedAt: new Date().toISOString() }
            : s
        );
      });
  ...
  const duplicateSignature = useCallback(
    async (id: string): Promise<EmailSignature | null> => {
  ...
  const getSignaturesByProvider = useCallback(
    (provider: SignatureProvider): EmailSignature[] => {
  ...
  const filterSignatures = useCallback(
    (options: SignatureFilterOptions): EmailSignature[] => {
      ...
      if (options.tags && options.tags.length > 0) {
        filtered = filtered.filter((s) => options.tags!.some((tag) => s.tags.includes(tag)));
      }
  ...
  const renderSignatureById = useCallback(
    (signatureId: string, format: 'html' | 'plain' = 'html'): string => {
  ...
  const logUsage = useCallback(
    (signatureId: string, emailId: string, emailType: 'new' | 'reply' | 'forward', recipientEmail: string): void => {
      const now = new Date().toISOString();
      ...
      // Update usage count
      saveSignatures((prev) =>
        prev.map((s) => (s.id === signatureId ? { ...s, usageCount: s.usageCount + 1, lastUsed: now } : s))
      );
  ...
  const getStats = useCallback((): SignatureStats => {
    ...
    const recentlyUsed = usageLogs.slice(0, 5).map((log) => ({
  ...
  const getTemplateById = useCallback(
    (id: string): SignatureTemplate | null => {
  ...
  const getTemplatesByCategory = useCallback(
    (category: SignatureTemplate['category']): SignatureTemplate[] => {
  ...
  const createFromTemplate = useCallback(
    async (templateId: string, data: Partial<CreateSignaturePayload>): Promise<EmailSignature | null> => {
  ...
  const addSocialLink = useCallback(
    async (signatureId: string, link: Omit<SignatureSocialLink, 'id'>): Promise<boolean> => {
      let found = false;
  ...
  const removeSocialLink = useCallback(
    async (signatureId: string, linkId: string): Promise<boolean> => {
      let found = false;
  ...
  // Import signatures
  const importSignatures = useCallback(
